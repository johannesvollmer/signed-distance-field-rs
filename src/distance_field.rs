...
#[inline]
pub fn length(x: i32, y: i32) -> f32 {
    let sqr_distance = x * x + y * y;
    (sqr_distance as f32).sqrt()
}

#[inline]
pub fn distance(x: u16, y: u16, target_x: u16, target_y: u16) -> f32 {
    length(x as i32 - target_x as i32, y as i32 - target_y as i32)
}

#[inline]
pub fn check_coordinates(x: i32, y: i32, width: u16, height: u16) -> bool {
...
impl SignedDistanceField {
    ...
    pub fn get_distance(&mut self, x: u16, y: u16) -> f32 {
        self.distances[self.flatten_index(x, y)].to_f32()
    }
    ...
    fn set_distance_target(&mut self, x: u16, y: u16, target_x: u16, target_y: u16) {
        ...
        self.distances[index] = f16::from_f32(distance(x, y, target_x, target_y));
    ...
    fn invert_distance_sign(&mut self, x: u16, y: u16) {
        let index = self.flatten_index(x, y);
        self.distances[index] = f16::from_f32( - self.distances[index].to_f32());
    }
    ...
    pub fn compute_approximate(binary_image: &impl BinaryImage) -> Self {
        ...
        let mut distance_field = SignedDistanceField {
            width, height,
            distances: vec![half::consts::INFINITY; width as usize * height as usize],
        ...
        for y in 0..height {
            for x in 0..width {
                if     crate::binary_image::is_at_edge(binary_image, x, y, -1,  0)
                    || crate::binary_image::is_at_edge(binary_image, x, y,  1,  0)
                    || crate::binary_image::is_at_edge(binary_image, x, y,  0, -1)
                    || crate::binary_image::is_at_edge(binary_image, x, y,  0,  1)
                {
